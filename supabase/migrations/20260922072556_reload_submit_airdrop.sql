/*
# Force PostgREST schema cache reload for submit_airdrop

The submit_airdrop function exists and is callable via SQL, but PostgREST's
schema cache does not see it (PGRST202 error). This migration re-creates
the function to force PostgREST to pick up the new signature. The function
body is identical to the existing one.
*/
